...
const MobileMenu = () => {
  ...
  return (
    <div className="relative">
      ...
      <motion.div
        className={`fixed top-0 left-0 h-full w-2/3 bg-white border-r border-gray-300 z-40 flex flex-col`}
        initial={{ x: '-100%' }}
        animate={{ x: isOpen ? 0 : '-100%' }}
        transition={{ type: 'spring', stiffness: 300 }}
      >
        <div className="flex items-center justify-center h-20 border-b border-gray-300">
        <Logo /> <span className='font-extrabold tracking-widest'>TrainingLand</span>
        </div>
